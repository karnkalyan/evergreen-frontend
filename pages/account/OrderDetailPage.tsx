...
// Variant Details Component - UPDATED to show shipping and remove stock
// const VariantDetails: React.FC<{ orderItems: any[]; currencySymbol: string }> = ({ orderItems, currencySymbol }) => {
//   if (!orderItems || orderItems.length === 0) return null;

//   return (
//     <div className="bg-white rounded-lg border border-gray-200 p-6">
//       <h3 className="text-lg font-semibold text-gray-900 mb-4">Product Variants & Shipping</h3>
//       <div className="space-y-4">
...
//                           <span className="text-gray-900">{variant.label}</span>
//                         </div>
//                         <div>
//                           <span className="font-medium text-gray-600">Price:</span>{' '}
//                           <span className="text-gray-900">{currencySymbol}{variant.price}</span>
//                         </div>
//                         {variant.mrp && variant.mrp > variant.price && (
//                           <div>
//                             <span className="font-medium text-gray-600">MRP:</span>{' '}
//                             <span className="text-gray-900 line-through">{currencySymbol}{variant.mrp}</span>
//                           </div>
//                         )}
//                       </div>
...
//                           <div className="font-medium text-blue-800 mb-2">Shipping Details</div>
//                           <div>
//                             <span className="font-medium text-blue-700">Shipping:</span>{' '}
//                             <span className="text-blue-900">{variantShipping.shipping}</span>
//                           </div>
//                           <div>
//                             <span className="font-medium text-blue-700">Country:</span>{' '}
//                             <span className="text-blue-900">{variantShipping.country}</span>
//                           </div>
//                           <div>
//                             <span className="font-medium text-blue-700">Currency:</span>{' '}
//                             <span className="text-blue-900">{variantShipping.currency}</span>
//                           </div>
//                         </div>
//                       )}
//                     </div>
...
//                     {item.quantity} × {currencySymbol}{(item.unitPrice || 0).toFixed(2)}
//                   </div>
//                 </div>
//               </div>
//             </div>
...
const OrderDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDynamicData = async () => {
      ...
      try {
        ...
        const [countriesData, paymentMethodsData, shippingOptionsData, companyInfoData] = await Promise.all([
          countryService.getCountries(),
          paymentMethodService.getPaymentMethods(),
          shippingService.getShippingOptions(),
          websiteSettingsService.getWebsiteSettings()
        ]);
        ...
        toast.error('Failed to load additional order information');
      } finally {
  ...
  // Get currency symbol from countries data
  const getCurrencySymbol = () => {
    if (!order?.currency) return '$';
    
    const country = countries.find(c => c.currency === order.currency);
    return country?.currencySymbol || order.currency;
  };

  // Get payment method details
  const getPaymentMethodDetails = () => {
    if (!order?.paymentMethodCode) return null;
    
    return paymentMethods.find(pm => pm.code === order.paymentMethodCode);
  };
  ...
  const getShippingMethodDetails = () => {
    ...
    return shippingOptions.find(so => 
      so.name === order.shippingMethod || so.code === order.shippingMethod
    );
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Variant Details - UPDATED with shipping information */}
            {/* <VariantDetails 
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                <Button className="w-full" variant="primary" onClick={() => setIsInvoiceOpen(true)}>
                  📄 Download Invoice
                </Button>
            ...
            {/* Customer Support */}
            <CustomerSupport companyInfo={companyInfo} />
          </div>
        </div>
      </div>
